...
REMOVE="remove" -- Constant to return when an effect should be removed
...
function Engine:run()
	...
	--[[
		...
		- nil, false: continue normally, no change
		- true: abort processing and go to next part
	   - "function", "table": replace(!) current item with the returned item
		- string REMOVE: remove current item in next frame
